...
<div class="p-4 max-w-3xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Attendant</h1>
        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Update attendant information</p>
    </div>

    <div class="glass-card">
        <form action="{{ route('organization.staff.update', $staff) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="event_ids[]" multiple size="5"
                        ...
                        <option value="{{ $event->id }}"
                                {{ in_array($event->id, old('event_ids', $staff->event_ids ?? [])) ? 'selected' : '' }}>
                            {{ $event->title }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="bg-cyan-50/50 dark:bg-cyan-900/20 rounded-lg p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="text-sm text-cyan-800 dark:text-cyan-300">
                        <p class="font-medium mb-1">Account Information</p>
                        <p><strong>Status:</strong> {{ ucfirst($staff->status) }}</p>
                        <p><strong>Last Login:</strong> {{ $staff->last_login_at ? $staff->last_login_at->format('M j, Y g:i A') : 'Never' }}</p>
                        <p><strong>Created:</strong> {{ $staff->created_at->format('M j, Y') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3 justify-end pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit" class="glass-btn-primary">Update Attendant</button>
            </div>
        </form>
    </div>
</div>
