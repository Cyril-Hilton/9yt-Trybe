...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-8">
        <a href="{{ route($pollRoutePrefix . '.show', $poll) }}" class="text-cyan-600 dark:text-cyan-400 hover:underline mb-4 inline-block">
            ← Back to Poll Details
        </a>
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Poll</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-2">Update your poll settings</p>
    </div>

    <!-- Form -->
    <form action="{{ route($pollRoutePrefix . '.update', $poll) }}" method="POST" enctype="multipart/form-data" class="glass-card rounded-2xl p-8">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <select name="company_id" id="company_id"
                ...
                <option value="{{ $company->id }}" {{ old('company_id', $poll->company_id) == $company->id ? 'selected' : '' }}>
                    {{ $company->name }}
                </option>
                @endforeach
            </select>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Leave blank to make this poll global.</p>
        </div>
        @endif
        ...
        <div class="mb-6">
            ...
            <input type="text" name="title" value="{{ old('title', $poll->title) }}" required
                   class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">
            @error('title')
        ...
        <div class="mb-6">
            ...
            <textarea name="description" rows="4"
                      class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">{{ old('description', $poll->description) }}</textarea>
            @error('description')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="event_id" id="event_id" class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">
                ...
                <option value="{{ $event->id }}" data-company="{{ $event->company_id }}" {{ old('event_id', $poll->event_id) == $event->id ? 'selected' : '' }}>
        ...
        <div class="mb-6">
            ...
            <select name="poll_type" required
                    class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">
                <option value="pageant" {{ old('poll_type', $poll->poll_type) == 'pageant' ? 'selected' : '' }}>Pageant</option>
                <option value="voting" {{ old('poll_type', $poll->poll_type) == 'voting' ? 'selected' : '' }}>Voting Poll</option>
                <option value="survey" {{ old('poll_type', $poll->poll_type) == 'survey' ? 'selected' : '' }}>Survey</option>
            </select>
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="voting_type" id="voting_type" required
                    class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">
                <option value="free" {{ old('voting_type', $poll->voting_type) == 'free' ? 'selected' : '' }}>Free</option>
                <option value="paid" {{ old('voting_type', $poll->voting_type) == 'paid' ? 'selected' : '' }}>Paid</option>
            </select>
        </div>
        ...
        <div class="mb-6" id="vote_price_section" style="display: none;">
            ...
            <input type="number" name="vote_price" value="{{ old('vote_price', $poll->vote_price) }}" step="0.01" min="0"
                   class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="flex items-center p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                <input type="checkbox" name="allow_multiple_votes" id="allow_multiple_votes" value="1" {{ old('allow_multiple_votes', $poll->allow_multiple_votes) ? 'checked' : '' }}
            ...
            <!-- Show Results -->
            <div class="flex items-center p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                <input type="checkbox" name="show_results" id="show_results" value="1" {{ old('show_results', $poll->show_results) ? 'checked' : '' }}
                       class="w-5 h-5 text-cyan-600 rounded focus:ring-cyan-500">
            ...
            <!-- Require Login -->
            <div class="flex items-center p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                <input type="checkbox" name="require_login" id="require_login" value="1" {{ old('require_login', $poll->require_login) ? 'checked' : '' }}
                       class="w-5 h-5 text-cyan-600 rounded focus:ring-cyan-500">
        ...
        <div class="mb-6">
            ...
            <input type="number" name="max_votes_per_user" value="{{ old('max_votes_per_user', $poll->max_votes_per_user) }}" min="1"
                   class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="datetime-local" name="start_date" value="{{ old('start_date', $poll->start_date ? $poll->start_date->format('Y-m-d\TH:i') : '') }}"
                       class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-900 dark:text-white mb-2">End Date (Optional)</label>
                <input type="datetime-local" name="end_date" value="{{ old('end_date', $poll->end_date ? $poll->end_date->format('Y-m-d\TH:i') : '') }}"
                       class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent">
            </div>
        </div>
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route($pollRoutePrefix . '.show', $poll) }}" class="px-6 py-3 bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white rounded-xl font-semibold hover:bg-gray-300 dark:hover:bg-gray-600 transition">
                Cancel
            </a>
        </div>
    </form>
