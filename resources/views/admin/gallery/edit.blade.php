...
<div class="p-8 max-w-3xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-white mb-2">Edit Gallery Image</h1>
        <p class="text-gray-400">Update gallery image details</p>
    </div>
    ...
    <form action="{{ route('admin.gallery.update', $image) }}" method="POST" enctype="multipart/form-data" class="bg-gray-800 rounded-xl p-6 border border-gray-700">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select name="category" required
                        class="w-full px-4 py-3 bg-gray-700 text-white border border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    <option value="new" {{ old('category', $image->category) === 'new' ? 'selected' : '' }}>New Events</option>
                    <option value="old" {{ old('category', $image->category) === 'old' ? 'selected' : '' }}>Old Events</option>
                </select>
            </div>
