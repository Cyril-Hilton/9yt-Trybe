...
<div class="p-8">
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-white mb-2">Magazine Management</h1>
                <p class="text-gray-400 text-sm sm:text-base">Manage magazine images</p>
            </div>
            <a href="{{ route('admin.magazine.create') }}" class="px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition shadow-lg flex items-center justify-center text-sm sm:text-base whitespace-nowrap">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        @php
            $total = \App\Models\MagazineImage::count();
            $active = \App\Models\MagazineImage::where('is_active', true)->count();
            $inactive = \App\Models\MagazineImage::where('is_active', false)->count();
        @endphp
    ...
    <div class="bg-gray-800 rounded-xl p-4 mb-6 border border-gray-700">
        <form action="{{ route('admin.magazine.index') }}" method="GET" class="flex flex-wrap gap-4">
            ...
            <a href="{{ route('admin.magazine.index') }}" class="px-6 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition">
                Clear
            </a>
            @endif
        </form>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 hover:border-indigo-500 transition group">
            ...
            <div class="p-4">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('admin.magazine.edit', $image) }}" class="flex-1 px-3 py-2 bg-indigo-600 text-white text-center rounded-lg hover:bg-indigo-700 transition text-sm">
                        Edit
                    </a>
                    <form action="{{ route('admin.magazine.toggle-active', $image) }}" method="POST" class="flex-1">
                        @csrf
                    ...
                    <form action="{{ route('admin.magazine.destroy', $image) }}" method="POST" onsubmit="return confirm('Delete this image? This action cannot be undone.');">
                        @csrf
                        @method('DELETE')
        ...
        <div class="col-span-full text-center py-20">
            ...
            <h3 class="text-xl font-semibold text-gray-400 mb-2">No magazine images found</h3>
            <p class="text-gray-500 mb-6">Get started by adding your first magazine image</p>
            <a href="{{ route('admin.magazine.create') }}" class="inline-flex items-center px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">
