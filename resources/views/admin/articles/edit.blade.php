...
<div class="p-8 max-w-4xl">
    ...
    <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="bg-gray-800 rounded-xl p-6 border border-gray-700 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="summary" rows="3" required
                      class="w-full px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600">{{ old('summary', $article->description) }}</textarea>
        </div>

        <div>
            <label class="block text-sm text-gray-300 mb-2">Content</label>
            <textarea name="content" rows="10" required
                      class="w-full px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600">{{ old('content', $article->content) }}</textarea>
        </div>
        ...
        <div>
            ...
            @if($article->image_url)
                <img src="{{ $article->image_url }}" alt="" class="mt-2 h-32 rounded-lg">
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <textarea name="meta_description" rows="2"
                          class="w-full px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600">{{ old('meta_description', $article->meta_description) }}</textarea>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="source_name" value="{{ old('source_name', $article->source_name) }}"
                       class="w-full px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600">
            </div>
            <div>
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_published" id="is_published" {{ $article->is_published ? 'checked' : '' }} class="h-4 w-4">
            <label for="is_published" class="text-sm text-gray-300">Publish</label>
        </div>
