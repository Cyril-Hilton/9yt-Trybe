...
<div class="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-gray-600">Manage and track your SMS campaigns</p>
                </div>
                <div class="flex space-x-3">
                    ...
                    <a href="{{ route('organization.sms.campaigns.send-single') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-bold rounded-xl hover:from-blue-700 hover:to-cyan-700 transform hover:scale-105 transition-all duration-200 shadow-lg">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/>
                        </svg>
                        Send Single SMS
                    </a>
                    <a href="{{ route('organization.sms.campaigns.send-bulk') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold rounded-xl hover:from-indigo-700 hover:to-purple-700 transform hover:scale-105 transition-all duration-200 shadow-lg">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl shadow-xl border-2 border-indigo-100 p-6 mb-6">
            <form method="GET" action="{{ route('organization.sms.campaigns.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <!-- Search -->
                <div>
                ...
                <div class="flex items-end space-x-2">
                    ...
                    <a href="{{ route('organization.sms.campaigns.index') }}" class="px-6 py-2 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-all duration-200">
                        Clear
                    </a>
                </div>
            </form>
                            ...
                            <div class="border-2 border-gray-200 rounded-xl p-6 hover:border-indigo-300 hover:shadow-lg transition-all duration-200">
                                <div class="flex items-start justify-between mb-4">
                                    ...
                                    <div class="ml-4 flex flex-col space-y-2">
                                        <a href="{{ route('organization.sms.campaigns.show', $campaign->id) }}" class="px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700 transition-colors duration-200 text-center">
                                            View Details
                                        </a>

                                        @if($campaign->status === 'scheduled')
                                            <form method="POST" action="{{ route('organization.sms.campaigns.cancel', $campaign->id) }}" onsubmit="return confirm('Are you sure you want to cancel this scheduled campaign?');">
                                                @csrf
                                                <button type="submit" class="w-full px-4 py-2 bg-yellow-600 text-white text-sm font-bold rounded-lg hover:bg-yellow-700 transition-colors duration-200">
                                                    Cancel
                                                </button>
                                            </form>
                                        @endif
                                        ...
                                        @if(in_array($campaign->status, ['completed', 'failed', 'cancelled']))
                                            <form method="POST" action="{{ route('organization.sms.campaigns.destroy', $campaign->id) }}" onsubmit="return confirm('Are you sure you want to delete this campaign?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="w-full px-4 py-2 bg-red-600 text-white text-sm font-bold rounded-lg hover:bg-red-700 transition-colors duration-200">
                                                    Delete
                                                </button>
                                            </form>
                                        @endif
                                ...
                                @if($campaign->status === 'completed' && $campaign->total_sent > 0)
                                    <div class="mt-4">
                                        <div class="flex items-center justify-between text-sm text-gray-600 mb-1">
                                            ...
                                            <span class="font-bold">{{ $campaign->delivery_rate }}%</span>
                                        </div>
                                        <div class="w-full bg-gray-200 rounded-full h-2">
                                            <div class="bg-gradient-to-r from-green-500 to-emerald-500 h-2 rounded-full" style="width: {{ $campaign->delivery_rate }}%"></div>
                                        </div>
                                    </div>
                                @endif
                            </div>
