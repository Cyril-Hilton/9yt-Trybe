        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-gray-600">Update your sender ID request</p>
                </div>
        ...
        @if($senderId->status === 'rejected')
            ...
            <div class="bg-red-50 border-2 border-red-200 rounded-2xl p-6 mb-8">
                <div class="flex items-start">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-red-900 mb-2">This request was rejected</h3>
                        <p class="text-sm font-semibold text-red-800 mb-1">Reason:</p>
                        <p class="text-sm text-red-700">{{ $senderId->rejection_reason }}</p>
                        <p class="text-sm text-red-800 mt-3">Please make the necessary changes and resubmit for review.</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="bg-white rounded-2xl shadow-xl border-2 border-indigo-100 overflow-hidden">
            <div class="bg-gradient-to-r from-indigo-600 to-purple-600 p-6">
                <h2 class="text-xl font-bold text-white flex items-center">
                    ...
                    Edit Sender ID Details
                </h2>
            </div>

            <form method="POST" action="{{ route('organization.sms.sender-ids.update', $senderId->id) }}" class="p-6 space-y-6" x-data="{ senderId: '{{ old('sender_id', $senderId->sender_id) }}' }">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="purpose"
                              ...
                              required
                              maxlength="500">{{ old('purpose', $senderId->purpose) }}</textarea>
                    @error('purpose')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="flex-1 text-sm text-yellow-800">
                            ...
                            <ul class="space-y-1 list-disc list-inside">
                                <li>Your updated sender ID request will be submitted for admin review again</li>
