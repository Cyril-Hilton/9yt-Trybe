...
@section('content')
<div class="max-w-md w-full mx-auto glass-effect p-6 sm:p-8 rounded-2xl shadow-2xl transition-all duration-300 transform hover:scale-[1.01]">
    <div class="text-center mb-8">
        <div class="inline-block bg-gradient-to-br from-cyan-500 to-blue-600 p-3 rounded-2xl shadow-lg mb-4 transform rotate-3 hover:rotate-6 transition-transform">
        ...
        <div class="mt-3 inline-flex items-center justify-center px-3 py-1 rounded-full bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 text-xs font-medium">
            Powering the trybe 🚀
        </div>
    </div>
    ...
    <div x-data="{
        ...
        <div class="flex gap-2 p-1 bg-gray-100 dark:bg-gray-800/80 rounded-xl mb-6">
            <button type="button"
                    ...
                    :class="loginMethod === 'email' ? 'bg-white dark:bg-gray-700 text-cyan-600 dark:text-cyan-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'"
            ...
            <button type="button"
                    ...
                    :class="loginMethod === 'phone' ? 'bg-white dark:bg-gray-700 text-cyan-600 dark:text-cyan-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'"
                    class="flex-1 px-4 py-2.5 rounded-lg font-medium transition-all text-sm flex items-center justify-center">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form x-show="loginMethod === 'email'" class="space-y-5" action="{{ route('organization.login') }}" method="POST"
            ...
            <div x-data="{ showPassword: false }">
                ...
                <div class="relative group">
                    ...
                    >
                        ...
                        <svg x-show="showPassword" x-cloak class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 24 24">
        ...
        <div class="flex items-center justify-between mt-2">
            ...
            <div class="text-sm">
                <a href="{{ route('organization.password.request') }}" class="font-medium text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 dark:hover:text-cyan-300 transition-colors">
                    Forgot password?
                </a>
            </div>
        </div>
    ...
    <form x-show="loginMethod === 'phone'" class="space-y-6"
          x-ref="phoneForm"
          action="{{ route('organization.send-otp') }}"
          method="POST"
          @submit="if (otpSent) { $el.action = '{{ route('organization.verify-otp') }}' } else { $el.action = '{{ route('organization.send-otp') }}' }"
          x-transition:enter="transition ease-out duration-300"
          x-transition:enter-start="opacity-0 transform scale-95"
          x-transition:enter-end="opacity-100 transform scale-100">
        @csrf
        ...
        <div x-show="otpSent" x-cloak class="space-y-4">
            ...
            <div class="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-200 px-4 py-3 rounded-lg text-sm flex items-center shadow-sm">
            ...
            <div>
                ...
                <input
                    ...
                    pattern="[0-9]{6}"
                    :required="otpSent"
                    class="appearance-none relative block w-full px-3 py-3 border border-cyan-300 dark:border-cyan-600 placeholder-gray-400 dark:placeholder-gray-600 text-gray-900 dark:text-white bg-cyan-50/50 dark:bg-gray-800/80 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 text-center text-3xl tracking-[0.5em] font-mono transition-all shadow-inner @error('otp') border-red-500 @enderror"
                    placeholder="000000"
        ...
        <div x-show="otpSent" x-cloak class="text-center space-y-4">
            <p class="text-sm text-gray-600 dark:text-gray-400">
                ...
                <button type="button" 
                        ...
                        class="font-medium text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 transition-colors underline decoration-2 underline-offset-2">
                    Resend OTP
                </button>
            </p>
    ...
    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700/50">
        ...
        <div class="mt-6 text-center">
            <a href="{{ route('admin.login') }}" class="inline-flex items-center px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-xs font-medium text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-gray-200 dark:hover:bg-gray-700 transition-all shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10">
