...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-4xl font-bold gradient-text mb-2">✏️ Edit Sender ID</h1>
            <p class="text-gray-600 dark:text-gray-400">Update your Sender ID request</p>
        </div>
        ...
        @if($senderId->status === 'rejected')
            <!-- Rejection Info -->
            <div class="mb-8 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 rounded-lg p-6">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <h4 class="text-sm font-bold text-red-800 dark:text-red-300 mb-2">This request was rejected</h4>
                        <p class="text-sm font-semibold text-red-700 dark:text-red-400 mb-1">Reason:</p>
                        <p class="text-sm text-red-600 dark:text-red-400">{{ $senderId->rejection_reason }}</p>
                        <p class="text-sm text-red-700 dark:text-red-400 mt-3">Please make the necessary changes and resubmit for review.</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        @if($senderId->status === 'approved')
            <!-- Approved Notice -->
            <div class="mb-8 bg-orange-50 dark:bg-orange-900/20 border-l-4 border-orange-500 rounded-lg p-6">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <h4 class="text-sm font-bold text-orange-800 dark:text-orange-300 mb-2">Editing Approved Sender ID</h4>
                        <p class="text-sm text-orange-700 dark:text-orange-400">This sender ID is currently approved. If you edit it, it will be reset to "Pending" status and will require admin re-approval before you can use it again.</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="space-y-4 mb-8">
            ...
            <div class="bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 p-6 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <h4 class="text-sm font-bold text-yellow-800 dark:text-yellow-300 mb-2">Important Notice</h4>
                        <p class="text-sm text-yellow-700 dark:text-yellow-400">Your updated Sender ID request will be submitted for admin review again. Status will be reset to "Pending" and you will be notified once it's approved or if it's rejected.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
            <form action="{{ route('user.sms.sender-ids.update', $senderId->id) }}" method="POST" x-data="{
                senderId: '{{ old('sender_id', $senderId->sender_id) }}',
                purpose: '{{ old('purpose', $senderId->purpose) }}',
                get isValidSenderId() {
